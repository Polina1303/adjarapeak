...
export type CatalogUiSortKey = "default" | "price-asc" | "price-desc" | "newest";
...
type CatalogUiText = {
  ...
  sort: string;
  sortOptions: Record<CatalogUiSortKey, string>;
  inStock: string;
  available: string;
  clearSearch: string;
  filters: string;
...
function enProductWord(count: number) {
  return count === 1 ? "item" : "items";
}

export const CATALOG_UI_TEXT: Record<Lang, CatalogUiText> = {
  RU: {
    ...
    searchAriaLabel: (placeholder, count) =>
      `${placeholder}, сейчас ${count} ${ruProductWord(count)}`,
  ...
  EN: {
    ...
    searchAriaLabel: (placeholder, count) =>
      `${placeholder}, currently ${count} ${enProductWord(count)}`,
  ...
  GE: {
    ...
    searchAriaLabel: (placeholder, count) =>
      `${placeholder}, ამჟამად ${count} პროდუქტი`,
