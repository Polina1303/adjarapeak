...
export const SalePage = () => {
  ...
  useEffect(() => {
    const currentItems = PRODUCT.filter((item) => {
      // Исправлено: проверяем, содержит ли массив types категорию товара
      return items[activeType].types.includes(item.category);
    });
  ...
  useEffect(() => {
    const filteredItems = PRODUCT.filter(
      (item) =>
        ...
        // Исправлено: проверяем принадлежность к выбранной категории
        (activeType > 0
          ? items[activeType].types.includes(item.category)
          : true)
    );
