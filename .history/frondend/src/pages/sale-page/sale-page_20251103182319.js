...
export const SalePage = () => {
  ...
  // При открытии /sale — автоматически переходим в первую категорию
  useEffect(() => {
    if (location.pathname === "/sale") {
      const firstCategory = CATEGORY_PRODUCT[0];
      setActiveCategory(0);
      navigate(`/sale/${firstCategory.path}`, { replace: true });
    } else {
      // Проверяем, если URL уже указывает на категорию
      const currentPath = location.pathname.split("/")[2];
      const foundIndex = CATEGORY_PRODUCT.findIndex(
        (c) => c.path === currentPath
      );
  ...
  // Клик по категории в верхнем меню
  const handleCategoryClick = (e) => {
    const categoryIndex = Number(e.key);
    setActiveCategory(categoryIndex);
    const category = CATEGORY_PRODUCT[categoryIndex];
      ...
      <div className="home-page__container-product">
        <div>
          ...
          <div className="sale-page-content">
            ...
            {/* Контент текущей категории/подкатегории */}
            <Outlet />
          </div>
        </div>
      </div>
