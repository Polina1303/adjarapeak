...
export const SalePage = () => {
  ...
  useEffect(() => {
    ...
    let filtered = PRODUCT.filter((product) => {
      // Если выбрана конкретная сабкатегория
      if (activeSubcategory) {
        return product.subcategory === activeSubcategory;
      }

      // Если сабкатегория не выбрана — показываем всё, что относится к категории
      const categoryMatch =
        product.category === category.types?.[0]?.category ||
        category.types?.some((t) => product.category === t.category);

      const subcategoryMatch = category.types?.some((t) =>
        t.subcategories?.some((sub) => sub.subcategory === product.subcategory)
      );
