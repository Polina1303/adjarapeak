...
"use client"; // ОБЯЗАТЕЛЬНО — если используешь useState, useRouter, onClick

import { Card, CardActionArea, CardContent, Skeleton } from "@mui/material";
import { useState } from "react";
import { useRouter } from "next/router"; // ПРАВИЛЬНО для Pages Router
...
export default function SaleCategoryPage() {
  ...
  const { section, category, subcategory } = router.query; // В Pages Router — router.query
  const [loadedIds, setLoadedIds] = useState([]);

  // Защита от undefined
  if (!section) return <p>Загрузка...</p>;
