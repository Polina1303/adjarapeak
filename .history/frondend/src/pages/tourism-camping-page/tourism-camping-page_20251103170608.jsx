...
export const TourismCampingPage = ({ category }) => {
  ...
  // Берём types, если они есть — это карточки, которые нужно показывать
  const cards = category.types || [];

  if (!cards.length) return <p>Нет доступных товаров в этом разделе</p>;

  return (
    <div className="home-page-product">
      {cards.map(({ title, img, category: cat, subcategories }) => {
        // используем category для пути
        const segment = cat || title.toLowerCase().replace(/\s+/g, "-");
