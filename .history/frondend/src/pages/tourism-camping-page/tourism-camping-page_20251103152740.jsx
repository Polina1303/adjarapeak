...
export const TourismCampingPage = ({ category }) => {
  ...
  // Если category — это массив разделов (главная /sale), показываем title всех разделов
  const cards = category.types ? category.types : category; // если category.types есть — берем их, иначе category массив

  return (
    <div className="home-page-product">
      {cards.map(({ title, img, category: cat, subcategory }) => {
        const segment =
          cat || subcategory || title.toLowerCase().replace(/\s+/g, "-"); // fallback на title
