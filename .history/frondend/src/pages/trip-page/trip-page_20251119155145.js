...
// Парсит строку типа "25 октября (сб)" или "25-26 октября" → Date или null (если "по запросу")
function parseEventDate(dateStr) {
  const trimmed = dateStr.trim();

  // События "по запросу" — всегда показываем
  if (trimmed.includes("В любой из дней") || trimmed.includes("по запросу")) {
    return null;
  }

  // Убираем день недели в скобках: "25 октября (сб)" → "25 октября"
  const clean = trimmed.replace(/\s*\([^)]*\)$/, "").trim();

  // Диапазон "25-26 октября" или просто "25 октября"
  const parts = clean.split(/\s+/);
  const dayStr = parts[0].split("-")[0]; // берём первое число из диапазона
  ...
  const filteredEvents = useMemo(() => {
    ...
    return events
      .filter((event) => {
        ...
        if (!eventDate) return true; // если не распарсили — покажем

        eventDate.setHours(0, 0, 0, 0);
        return eventDate >= now; // только будущие и сегодня
      })
      .filter((event) => {
