...
function parseEventDate(dateStr) {
  ...
  // Всегда показываем "по запросу"
  if (trimmed.includes("В любой из дней") || trimmed.includes("по запросу")) {
    return null;
  }

  // Убираем день недели: "15 ноября (сб)" → "15 ноября"
  const clean = trimmed.replace(/\s*\([^)]*\)$/, "").trim();
  const parts = clean.split(/\s+/);
  const dayStr = parts[0].split("-")[0]; // поддержка "15-16 ноября"
  ...
  // КЛЮЧЕВОЙ ФИКС: обнуляем время у текущей даты для корректного сравнения по дням
  const now = new Date();
