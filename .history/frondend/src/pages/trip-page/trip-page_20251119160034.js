...
export const TripPage = () => {
  ...
  const filteredEvents = useMemo(() => {
    const today = new Date();
    today.setHours(0, 0, 0, 0); // Сегодняшняя дата без времени

    const upcoming = events.filter((event) => {
      // "По запросу" — пропускаем фильтр по дате
      if (
      ...
      if (!eventDate) return true; // Если не распарсили — показываем (на всякий)

      // СТРОГОЕ СРАВНЕНИЕ ПО ДНЯМ: >= сегодня
      return eventDate >= today;
    });

    console.log(
      `Сегодня: ${today.toDateString()}. Будущих событий: ${upcoming.length}`
    ); // Для дебага
