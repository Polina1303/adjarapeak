...
function parseEventDate(dateStr) {
  ...
  // "По запросу" — всегда показываем (null = не фильтруем по дате)
  if (trimmed.includes("В любой из дней") || trimmed.includes("по запросу")) {
    return null;
  }

  // Убираем скобки с днём недели: "15 ноября (сб)" → "15 ноября"
  const clean = trimmed.replace(/\s*\([^)]*\)\s*$/, "").trim();

  // Парсим: "15 ноября" или "15-16 ноября" (берём первое число)
  const parts = clean.split(/\s+/);
  if (parts.length < 2) return null;

  const dayStr = parts[0].split("-")[0]; // Первое число из диапазона
  ...
  if (isNaN(day) || month === undefined || day < 1 || day > 31) {
    console.warn(`Не удалось распарсить дату: ${dateStr}`); // Для дебага
    return null;
  }

  const year = new Date().getFullYear();
  let eventDate = new Date(year, month, day, 0, 0, 0, 0); // Обнуляем время сразу

  // Получаем сегодняшнюю дату с обнулением времени
  const today = new Date();
  today.setHours(0, 0, 0, 0);

  if (eventDate < today) {
    eventDate.setFullYear(year + 1);
    console.log(`Перенос даты ${dateStr} на ${year + 1} год`); // Для дебага
  }

  console.log(`Дата события "${dateStr}" → ${eventDate.toDateString()}`); // Для дебага
  return eventDate;
}
