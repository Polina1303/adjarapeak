...
export const CartBlock = () => {
  ...
  // Загружаем данные только после монтирования (только клиент)
  useEffect(() => {
    if (typeof window !== "undefined") {
      setMounted(true);
      const storedItems = JSON.parse(localStorage.getItem(CART_KEY) || "[]");
  ...
  // Сохраняем изменения корзины в localStorage
  useEffect(() => {
    if (mounted && typeof window !== "undefined") {
      localStorage.setItem(CART_KEY, JSON.stringify(items));
    }
  ...
  // 🚫 убрал useMemo c localStorage — он не нужен и ломает SSR

  // 💡 Можно вообще не рендерить корзину, пока компонент не смонтировался:
  ...
  return (
    <div className={styles["cart-block"]}>
      <Button
        ...
        onClick={() => setIsCartMenuVisible(!isCartMenuVisible)}
      >
        <BsCart3 size={22} className={styles["cart-icon"]} />
        {totalCount > 0 && <ItemsInCart quantity={totalCount} />}
        {totalPrice > 0 && (
          <span className={styles["cart-price"]}>{totalPrice}₾</span>
        )}
      </Button>
