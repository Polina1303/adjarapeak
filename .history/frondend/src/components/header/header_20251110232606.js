...
export const Header = () => {
  ...
  // Используем дефолтное значение для серверного рендеринга
  const defaultLanguage = "ENG"; // или "RU" - выберите то, что должно быть по умолчанию
  ...
  // Обновление checked при изменении languages
  useEffect(() => {
    if (mounted) {
      setChecked(languages === "RU");
    }
  ...
  // Обновление языка при изменении checked
  useEffect(() => {
    if (mounted) {
      dispatch(updateCurrentLanguages(checked ? "RU" : "ENG"));
    }
  ...
  // Используем дефолтный язык до монтирования
