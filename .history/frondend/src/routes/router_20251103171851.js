...
import { TourismCampingPage } from "../pages/tourism-camping-page";
...
export const router = createBrowserRouter([
  {
    ...
    children: [
      ...
      //     },
      //     {
      //       path: `${CATEGORY_PRODUCT[0].path}`,
      //       Component: TourismCampingPage,
      //       children: [
      //         {
      //           path: `${CATEGORY_PRODUCT[0].types[0].category}`,
      //           element: <p>Hi</p>,
      //         },
      //       ],
      //     },
      ...
      //       index: true,
      //       Component: () => (
      //         <TourismCampingPage category={CATEGORY_PRODUCT[0]} />
      //       ),
      //     },
      //     // Родительский путь
      //     {
      //       path: CATEGORY_PRODUCT[0].path, // "tourismCamping"
      //       Component: () => (
      //         <TourismCampingPage category={CATEGORY_PRODUCT[0]} />
      //       ),
      //       children: CATEGORY_PRODUCT[0].types.map((type) => ({
      //         path: type.category, // "tent", "sleepingbag" и т.д.
      //         Component: () => <TourismCampingPage category={type} />,
      //         children: type.subcategories
      //           ? type.subcategories.map((sub) => ({
      //               path: sub.subcategory, // "tent_accessories", "sleepingbag"
      //               Component: () => <TourismCampingPage category={sub} />,
      //             }))
      //           : [],
      //       })),
      //     },
      ...
      // },
      {
        ...
        Component: SalePage,
        children: [
          // Главная страница продаж — рендерим первую категорию
          {
            index: true,
            Component: () => (
              <TourismCampingPage category={CATEGORY_PRODUCT[0]} />
          ...
          ...CATEGORY_PRODUCT.map((section) => ({
            path: section.path,
            Component: () => <TourismCampingPage category={section} />,
            // Вложенные маршруты — типы (Палатки, Спальники и т.д.)
            children: section.types
              ? section.types.map((type) => ({
                  path: type.category,
                  Component: () => <TourismCampingPage category={type} />,
                  // Ещё глубже — подкатегории (например, Палатки → Аксессуары для палаток)
                  children: type.subcategories
                    ? type.subcategories.map((sub) => ({
      ...
      { path: "app/:id/delivery_terms", Component: DeliveryTerms },
