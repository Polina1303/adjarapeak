...
import { CATEGORY_PRODUCT } from "../../../src/components/product-range/categoryProduct";
...
export default function SaleCategoryPage() {
  ...
  if (section) {
    if (subcategory) {
      ...
      const typeCategories = section.types.map((t) => t.category);
      filteredProducts = PRODUCT.filter(
        (p) =>
          typeCategories.includes(p.category) ||
          typeCategories.includes(p.subcategory)
      );
    }
  }
